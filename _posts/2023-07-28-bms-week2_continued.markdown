...
We may like to measure the size of a set of continuous functions (e.g. functions in $C[0,1]$). It turns out that we can do this by only using the sample paths of a Brownian Motion, since for each $\omega\in \Omega$, $B_t(\omega): t \mapsto B_t(\omega)$ is a continuous function. The intuition is that the probability of this given Brownian Motion covers the set is gives the size of the set.  

To properly define the Wiener measure, we need some constructions. Let $C(\mathbb{R}\_{+}, \mathbb{R})$ be the set of continuous functions from $\mathbb{R}_+$ to $\mathbb{R}$. Define the $\sigma-$algebra $\mathcal{L}$ as the smallest $\sigma-$algebra such that for all $t\geq 0$, the projection map $t \mapsto w(t)$ is measurable. Now let $\Omega$ be the usual sample space, we consider:  
...
> (Indeed, let the map above be $f$ and $f \circ g_t=B_t$, so $f^{-1}(g_t^{-1}(B))$ is measurable, which gives the desired result, as $g_t^{-1}(B)$ form the generator of sigma algebra on $C(\mathbb{R}_+, \mathbb{R})$).  
...
$$
\begin{aligned}
W(A) &= P(B_{t_0}\in A_0, \dots, B_{t_n} \in A_n) \\
...
The consequence of the preceeding formula is that the Wiener measure (the law of the Brownian motion) is uniquely defined. So for distinct Brownian motions $B_1, B_2$, we have $P(B_1 \in A) = P(B_2 \in A)$ for some $A \in \mathcal{L}$, this will be important in applications, as we have various modifications that have the same distribution as the Brownian motion.  
...
This is really a section about the sample path properties like hitting particular values, but most of these follow from the Blumenthal 0-1 Law, which we will prove first.  

Define $\mathcal{F}_t = \sigma(B_s: s \leq t)$, the natural filtration of the Brownian motion and
$$
...
<div class="transparent-box">
   ...
   The $\sigma-$algebra $\mathcal{F}_{0+}$ is trivial, i.e. for all $A \in \mathcal{F}_{0+}$, $P(A) \in \{0,1\}$.
</div>    
...
Now simply note $P(A)=\lim P(\sup_{0\leq s \leq \varepsilon_p} B_s > 0) \geq P(B\_{\varepsilon_p}>0)=1/2$ by symmetry, so $P(A)=1$. The other assertion is proved by replacing $B_s$ with $-B_s$.   
...
We show this by first noting that a continuous function $f$ takes any value in $\mathbb{R}$ if and only if $\limsup f = \infty$ and $\liminf f = -\infty$. We now show the probability $P(\sup\_{t\geq 0} B_t > M)=1$ for all $M>0$.  

Note from the previous property, we have $P(\sup\_{0\leq t \leq 1} B_t > 0 ) = \lim\_{\delta \to 0} P(\sup\_{0\leq t \leq 1} B_t > \delta) =1$, using a rescaling of Brownian motion $B_t^{\delta} = 1/\delta B_{\delta^2 t}$, we have $P(\sup\_{0\leq t \leq 1/\delta^2} B_t^{\delta} > 1 ) = 1$, using the definition of [Wiener measure](#wiener-measure), we have for any $M>0$:  

$$
\begin{aligned}
\lim_{\delta \to 0} P(\sup_{0\leq t \leq 1/\delta^2} B_t^{\delta} > 1) &=
\lim_{\delta \to 0} P(\sup_{0\leq t \leq 1/\delta^2} B_t> 1) \\
&= \lim_{\delta \to 0} P(\sup_{0\leq t \leq M^2/\delta^2} B_t > M) \\
\end{aligned}
$$

This shows $P(\sup\_{t\geq 0} B_t > M)=1$ for all $M>0$ and for the infimum, we can use $-B_t$ instead.  
...
We first define the **stopping time** $T$, which is a random variable in $[0, \infty]$ such that $\{T \leq t\} \in \mathcal{F}_t$ for all $t\geq 0$.  
...
It is easy to see $T$ is measurable wrt. $\mathcal{F}_T$. Now we claim the random variable $\mathbf{1}\_{T<\infty} B_T$ is $\mathcal{F}_T-$measurable. Indeed, we can write it as a pointwise limit of simple functions:    
...
Again, we only sketch the proof here. It suffices to show that $E[\mathbf{1}_A F(B\_{t_1}^{(T)}, \ldots, B\_{t_n}^{(T)})] = E[\mathbf{1}_A] E[F(B\_{t_1}^{(T)}, \ldots, B\_{t_n}^{(T)})]$ for all $A \in \mathcal{F}_T$ and $F$ bounded and measurable. The independence and distribution follows similarly to the [Blumenthal 0-1 Law](#blumenthal-0-1-law).  
...
$$
\begin{aligned}
E[\mathbf{1}_A F(B_{t_1}^{(T)}, \ldots, B_{t_n}^{(T)})] &= \lim_{n\to \infty} E[\mathbf{1}_A F(B_{t_1}^{[T]_n}, \ldots, B_{t_p}^{[T]_n})] \\
&= \lim_{n\to \infty} \sum_{k=0}^\infty E[\mathbf{1}_A \mathbf{1}_{\{k2^{-n} \leq T < (k+1)2^{-n}\}} F(B_{k2^{-n}+t_1}-B_{k2^{-n}}, \ldots, B_{k2^{-n}+t_p}-B_{k2^{-n}}] \\
\end{aligned}
$$

where we use $[T]_n$ to denote the smallest integer $k$ such that $k2^{-n} \leq T < (k+1)2^{-n}$ and we used dominated convergence theorem.  
...
An important application is the reflection principle of the Brownian motion.  
...
<div class="transparent-box">
   ...
   Let $B$ be a Brownian motion and $a>0$, define $S_t = \sup_{s\leq t} B_s$, then for $a\geq 0$ and $b\leq a$, we have
    $$
    P(S_t \geq a, B_t \leq b) = 2P(B_t \geq 2a-b)
    $$  
...
$$
P(S_t \geq a, B_t \leq b) = P(T_a \leq t, B_t \leq b) = P(T_a \leq t, B_{t-T_a}^{(T_a)} \leq b-a)
$$  

since $B\_{t-T_a}^{(T_a)} = B_t - B_{T_a}=B_t - a$ on the event $\\{T_a \leq t\\}$. Now we can use the strong Markov property to get independence between $T_a$ and $B\_{t-T_a}^{(T_a)}$, so replacing $B\_{t-T_a}^{(T_a)}$ with a random variable with the same distribution is allowed and we can write  

$$
\begin{aligned}
P(T_a \leq t, B_{t-T_a}^{(T_a)} \leq b-a) &= P(T_a \leq t, -B_{t-T_a}^{(T_a)} \leq b-a) \\
&= P(T_a \leq t, B_t \geq 2a-b) \\
&= P(B_t \geq 2a-b) \\
\end{aligned}
$$ 
...
The distribution of $T_a$ then follows as $\frac{a^2}{B_1^2}$, as   
...
$$
E[\mathbf{1}_A F(B_{t_1}, \ldots, B_{t_n})] = \lim_{\varepsilon\to 0} E[\mathbf{1}_A F(B_{t_1}-B_{\varepsilon}, \ldots, B_{t_n}-B_{\varepsilon})]
$$
